...
//        }
        ...
        $this -> Handle = new Spreadsheet_Excel_Reader($Filepath, true, 'UTF-8');
    ...
    public function ChangeSheet($Index)
    {
        ...
        if (isset($this -> Sheets[$Index]))
        {
            ...
            $this -> ColumnCount = $this -> Handle
                -> sheets[$this -> CurrentSheet]['numCols'];
            $this -> RowCount = $this -> Handle ->
            sheets[$this -> CurrentSheet]['numRows'];
            // For the case when Spreadsheet_Excel_Reader doesn't have the row count set correctly.
            if (!$this -> RowCount && count($this -> Handle
                    -> sheets[$this -> CurrentSheet]['cells']))
            {
                end($this -> Handle -> sheets[$this
                    -> CurrentSheet]['cells']);
                $this -> RowCount = (int)key($this -> Handle
                    -> sheets[$this -> CurrentSheet]['cells']);
            }
            if ($this -> ColumnCount)
            {
                $this -> EmptyRow = array_fill(1,
                    $this -> ColumnCount, '');
            }
            else
            {
    ...
    public function next()
    {
        ...
        elseif (isset($this
                -> Handle -> sheets[$this -> CurrentSheet]['cells'][$this -> Index]))
        {
