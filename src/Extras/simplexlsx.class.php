...
//		$this->styles = array();
    ...
    public function titles(){
        $ws = $this->worksheet( 0 );
            ...
            $header = $ws->sheetData->row[0];
            $cur = [];
        foreach($header->c as $c){
            $cur[] = utf8_encode($this->value( $c )) ;
        }

    return $cur;
    }
	public function rows($withHeader =true ,$skipRows = false , $worksheet_id = 0 ) {
		...
		foreach ( $ws->sheetData->row as $row ) {
			...
			$curR ++;
			if($skipRows!==  false and $skipRows > 0 && count($rows) == $skipRows ){
                return $rows;
            }
		}
